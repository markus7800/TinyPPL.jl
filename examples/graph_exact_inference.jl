...
Random.seed!(0)
@time X1, P1, messages1 = sample_junctiontree_naive(res, :dfs);
print_messages_Z(res, X1, messages1)


Random.seed!(0)
@time X2, P2 = sample_junctiontree(res);

X1 == X2
P1 ≈ P2

P2 ≈ get_posterior_density(res, X2)

function eval_true_posterior_density(model, res, X)
    variables = reduce(∪, map(node -> node.cluster, res.junction_tree))
    ix_to_variable = Dict(var.variable => var for var in  variables)

    _X = zeros(model.n_latents)
    for (i, v) in X
        _X[i] = ix_to_variable[i].support[v]
    end
    return exp(model.logpdf(_X, model.observations)) / res.evidence
end
eval_true_posterior_density(model, res, X1)


Random.seed!(0)
@time X3, P3 = sample_junctiontree_optimised_messaging(res);
P3 ≈ get_posterior_density(res, X3)
