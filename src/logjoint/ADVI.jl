...
function advi_logjoint_tracker_2(logjoint::Function, n_samples::Int, L::Int, learning_rate::Float64, q::VariationalDistribution, estimator::ELBOEstimator)
    phi = no_grad(get_params(q))
...
function advi_logjoint_fwd(logjoint::Function, n_samples::Int, L::Int, learning_rate::Float64, q::VariationalDistribution, estimator::ELBOEstimator)
    phi = no_grad(get_params(q))
    ...
    @progress for i in 1:n_samples
        grad = ForwardDiff.gradient(objective, phi)
        # decayed adagrad update rule
        acc = @. post * acc + pre * grad^2
        rho = @. learning_rate / (sqrt(acc) + eps)
        phi += @. rho * grad
...
export advi_logjoint_fwd

import ReverseDiff
function advi_logjoint_rws(logjoint::Function, n_samples::Int, L::Int, learning_rate::Float64, q::VariationalDistribution, estimator::ELBOEstimator)
    phi = no_grad(get_params(q))
...
export advi_logjoint_rws


function advi_logjoint_rws_2(logjoint::Function, n_samples::Int, L::Int, learning_rate::Float64, q::VariationalDistribution, estimator::ELBOEstimator)
    phi = no_grad(get_params(q))
    ...
    local tape::ReverseDiff.InstructionTape
    @progress for i in 1:n_samples

        tape = ReverseDiff.InstructionTape()
        phi_tracked = ReverseDiff.track(phi, tape)

        q = update_params(q, phi_tracked)
        elbo = estimate_elbo(estimator, logjoint, q, L)

        
        ReverseDiff.pull_value!(elbo)
        ReverseDiff.unseed!(phi_tracked) # sets derivatives to 0
        ReverseDiff.seed!(elbo) # sets derivatives to 1
        ReverseDiff.reverse_pass!(tape)
        grad= ReverseDiff.deriv(phi_tracked)
...
export advi_logjoint_rws_2

export advi_logjoint, advi_meanfield_logjoint, advi_fullrank_logjoint, advi_logjoint_tracker_2