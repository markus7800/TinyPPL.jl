...
function jacobian(tt::TraceTransformation, old_trace::Dict{Any,Real}, new_trace::Dict{Any,Real})
    function wrapped_f(X::AbstractVector{<:Real})
        ...
        return Tracker.collect([v for (addr,v) in tracked_new_tr if addr in tt.continuous_writes])
    end
    continuous_input_arr = Float64[v for (addr, v) in old_trace if addr in tt.continuous_reads]
    # continuous_input_arr will be tracked automatically
...
function jacobian(tt::TraceTransformation,
    ...
    function wrapped_f(X::AbstractVector{<:Real})
        ...
        return Tracker.collect([v for (addr,v) in tracked_new_model_tr ∪ tracked_new_proposal_tr if addr in tt.continuous_writes])
    end
